...
- `TestnetGramDistributor` sends exactly 10 test GRAM for a server-authorized voucher and permanently blocks a second claim by the same private Telegram subject or wallet address.
...
`ton:testnet:grant:deploy` deploys the separate 10-GRAM distributor. It cannot mint native test GRAM: fund its address with 10 GRAM per intended participant plus its immutable reserve and transaction fees. The signing seed is server-only, testnet-only, and must never be reused as a wallet seed.
