  ...
  const status = /session/i.test(message)
    ...
    : /прав/i.test(message)
      ...
      : /уже подтвержд|duplicate|unique/i.test(message)
        ? 409
        : /не найден|закрыт|истёк|вариант|адрес|сеть|срок|кворум|описан|назван|подпис|кошел|фонд|дол|капитал|сумм|назначен|тип финансов|снимок|реестр/i.test(message)
          ? 400
          : 503;
  return noStore({
    ok: false,
