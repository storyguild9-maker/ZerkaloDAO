...
const AVATAR_SEAT_COUNT = ACTIVE_AVATAR_ID_LIST.length;
const AVATAR_ASSET_VERSION = "seated-variants-20260712-0635";
...
export function MeshySceneConstructor({ plain = false }: { plain?: boolean } = {}) {
  ...
  const applySeatedAvatarAdjustmentsToScene = (map = avatarSeatAdjustmentsRef.current) => {
    ...
    initiateAvatars.filter((avatar) => ACTIVE_AVATAR_IDS.has(avatar.id)).forEach((avatar, index) => {
  ...
  useEffect(() => {
    ...
    if (avatarMotion !== "walk-to-seat" && avatarMotion !== nextMotion) {
  ...
  useEffect(() => {
    ...
    const getMotionUrl = (avatar: InitiateAvatar, motion: string) => {
      if (motion === "sit-at-table") {
        const preferredSeatedMotion = getAvatarSeatedMotionId(avatar);
        const seatedUrl = avatar.animationTasks?.[preferredSeatedMotion]?.localModel ?? avatar.animationTasks?.["sit-at-table"]?.localModel ?? avatar.animationTasks?.["walk-to-seat"]?.localModel;
        return versionedAvatarUrl(seatedUrl ?? "");
      }
      if (motion === "basic-walking") {
        return versionedAvatarUrl(avatar.basicAnimations?.walking ?? `/models/initiates/animations/${avatar.id}-basic-walking.glb`);
      }
      const explicitUrl = avatar.animationTasks?.[motion]?.localModel;
      if (explicitUrl) return versionedAvatarUrl(explicitUrl);
      return versionedAvatarUrl(avatar.basicAnimations?.walking ?? avatar.riggedModel ?? avatar.localModel ?? "");
    };
    const placeAvatar = async (avatar: InitiateAvatar, index: number, total: number, controlled: boolean) => {
      const motion = controlled ? avatarMotion : "sit-at-table";
      const url = getMotionUrl(avatar, motion);
      if (!url) return;
      const gltf = await loadGltf(url);
      if (!gltf || cancelled) return;

      const root = new THREE.Group();
      root.name = controlled ? "controlled-initiate-avatar" : `seated-initiate-${avatar.id}`;
      ...
      if (controlled && avatar.riggedModel && avatar.riggedModel !== url) {
        const idleGltf = await loadGltf(avatar.riggedModel);
      ...
      const forceSeatOnLoad = controlled && seatAllAvatarsOnNextLoadRef.current;
      ...
      const animationClip = gltf.animations[0] ? sanitizeAvatarAnimationClip(gltf.animations[0], motion) : null;
      ...
      if (action) {
        if (controlled) {
          ...
          action.play();
          action.paused = true;
          controlledAvatarRef.current = { root, model, idleModel, seatedModel, mixer, action, seatedMixer, seatedAction, baseModelPosition: model.position.clone(), baseIdleModelPosition, baseSeatedModelPosition, yaw: root.rotation.y - modelFacingOffset, wasMoving: false, isSeated: false, seating: null };
        } else {
      ...
      if (controlled && controlledAvatarRef.current) {
        ...
        const seatedUrl = getMotionUrl(avatar, "sit-at-table");
        if (seatedUrl) {
          void loadGltf(seatedUrl).then((seatedGltf) => {
            ...
            const seatedClip = seatedGltf.animations[0] ? sanitizeAvatarAnimationClip(seatedGltf.animations[0], "sit-at-table") : null;
    ...
    const loadInitiates = async () => {
      try {
        const response = await fetch("/models/initiates/manifest.json", { cache: "no-store" });
        ...
        const controlled = allAvatars.find((avatar) => avatar.id === controlledAvatarId) ?? allAvatars.find((avatar) => avatar.id === DEFAULT_CONTROLLED_AVATAR_ID) ?? allAvatars[0];
    ...
    return () => {
      ...
      if (controlledAvatarRef.current) {
        const avatarId = controlledAvatarRef.current.root.userData.avatarId;
        if (typeof avatarId === "string") {
          controlledAvatarPoseRef.current[avatarId] = { position: controlledAvatarRef.current.root.position.clone(), yaw: controlledAvatarRef.current.yaw };
        }
      }
      controlledAvatarRef.current = null;
      avatarMixersRef.current = [];
  ...
  }, [avatarMotion, controlledAvatarId, isReady, plain, templateRevision]);
  ...
  const seatAllAvatarsAtChairs = () => {
    const avatars = initiateAvatars.filter((avatar) => ACTIVE_AVATAR_IDS.has(avatar.id));
    if (avatars.length === 0) {
  ...
  return (
    <section className={`meshy-constructor ${plain ? "meshy-constructor--plain" : ""} ${panelsHidden ? "is-clean" : ""}`} aria-label="3D конструктор Meshy деталей">
      ...
      <header className="meshy-constructor__topbar">
        ...
        <nav aria-label="Навигация 3D конструктора">
          ...
          <select aria-label="Выбор управляемого аватара" className="meshy-avatar-motion-select" onChange={(event) => { const nextAvatar = initiateAvatars.find((avatar) => avatar.id === event.target.value) ?? null; setControlledAvatarId(event.target.value); setAvatarMotion(getDefaultAvatarMotion(nextAvatar)); }} title="Кем управлять" value={controlledAvatarId}>
            {initiateAvatars.length > 0 ? initiateAvatars.map((avatar) => <option key={avatar.id} value={avatar.id}>{avatar.title || avatar.id}</option>) : <option value={controlledAvatarId}>Аватар</option>}
          </select>
        ...
        <div className="meshy-constructor__section meshy-avatar-seats">
          ...
          <div className="meshy-seat-editor">
            ...
            <select aria-label="Выбор аватара для настройки посадки" className="meshy-constructor__search" onChange={(event) => setSeatEditorAvatarId(event.target.value)} value={seatEditorAvatar?.id ?? seatEditorAvatarId}>
              {initiateAvatars.map((avatar) => <option key={avatar.id} value={avatar.id}>{avatar.title || avatar.id}</option>)}
            </select>
            {seatEditorAvatar ? (
              <div className="meshy-seat-tuning">
