...
def hand_world_matrix(armature):
    return armature.matrix_world @ armature.pose.bones["RightHand"].matrix
...
def set_prop_world_matrix(prop, armature, world_matrix):
    prop.parent = None
    prop.matrix_world = world_matrix
    bpy.context.view_layer.update()
    return hand_world_matrix(armature).inverted() @ world_matrix
...
def place_spear(prop, armature, action):
    ...
    return set_prop_world_matrix(
        prop,
        armature,
        oriented_matrix(origin, direction, scale, math.radians(12)),
    )
...
def place_sword(prop, armature, action):
    ...
    scale = 0.58
    grip = -0.68
    direction = Vector((0.08, 0.04, -0.996)).normalized()
    origin = right - direction * (grip * scale)
    return set_prop_world_matrix(
        prop,
        armature,
        oriented_matrix(origin, direction, scale, math.radians(-18)),
    )
...
def create_transform_action(prop, armature, clip_name, character_action, bone_name, bone_offset, visible):
    ...
    for frame in range(start, end + 1):
        ...
        prop.scale = tuple(value * multiplier for value in scale)
        prop.keyframe_insert(data_path="location", frame=frame, group="Hand Follow")
        prop.keyframe_insert(data_path="rotation_quaternion", frame=frame, group="Hand Follow")
        prop.keyframe_insert(data_path="scale", frame=frame, group="Visibility")
    prop.animation_data.action = None
    return action


def configure_nla(armature, props, character_actions, attachments):
    ...
    for clip_name, action in character_actions.items():
        for prop in props:
            ...
            prop_actions[(clip_name, prop.name)] = create_transform_action(
                prop,
                armature,
                clip_name,
                action,
                bone_name,
                bone_offset,
                prop.name == BACK_WEAPON or TARGETS.get(clip_name) == prop.name,
            )
    ...
    for clip_name, action in character_actions.items():
        ...
        for prop in props:
            add_nla_strip(prop, clip_name, prop_actions[(clip_name, prop.name)])
...
def main():
    ...
    axe = import_prop(axe_path, BACK_WEAPON)
    sword = import_prop(sword_path, "Weapon_Sword")

    attachments = {
        spear.name: ("RightHand", place_spear(spear, armature, character_actions["Axe_Stance"])),
        sword.name: ("RightHand", place_sword(sword, armature, character_actions["Idle_5"])),
        axe.name: ("Spine02", place_axe_on_back(axe, armature, character_actions["Idle_02"])),
    }
    props = [spear, axe, sword]
    configure_nla(armature, props, character_actions, attachments)
    ...
    report = {
        ...
        "attachment": "baked hand transforms with a permanent Spine02 back axe",
    }
